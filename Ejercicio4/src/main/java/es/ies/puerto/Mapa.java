...
class Mapa {
    ...
    public synchronized boolean moverCazador(Cazador cazador, String nuevaUbicacion) {
        ...
        if (isEnCueva(nuevaUbicacion)) {
            System.out.println(cazador.getNombre() + " no puede entrar en la cueva.");
            return false; 
        }
    ...
    public void mostrarMapa() {
        ...
        if (cuevaUbicacion != null) {
            String[] coords = cuevaUbicacion.split(",");
            int x = Integer.parseInt(coords[0]);
            int y = Integer.parseInt(coords[1]);
            mapaVisual[x][y] = "U"; // 'U' representa la cueva
        }
    ...
    private void crearCuevaAleatoria() {
        Random random = new Random();
        // Generar coordenadas aleatorias
        int x = random.nextInt(size);
        int y = random.nextInt(size);
        cuevaUbicacion = x + "," + y;
        System.out.println("Cueva creada en la ubicación: " + cuevaUbicacion);
    }

    public boolean isEnCueva(String ubicacion) {
        return ubicacion.equals(cuevaUbicacion);
    }
